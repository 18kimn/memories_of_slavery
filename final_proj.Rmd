...
```{r}
fips_shps <- readRDS("data/county shapefile.RDS") %>% 
  st_as_sf() %>% 
  mutate(fips = paste0(STATEFP, COUNTYFP)) %>% 
  select(fips)

#aggregating total slave pop by county
slaves <- read_csv("data/nhgis0002_ds14_1860_county.csv") %>% 
...
old <- st_read("data/nhgis/US_county_1860_conflated.shp",
                   quiet= T) %>%
...
```{r, eval = F}
slaves <- st_read("data/nhgis/US_county_1860_conflated.shp",
                   quiet= T) %>% 
  select(GISJOIN) %>% 
...
epa15 <- vroom("data/EJSCREEN_20150505.csv", 
